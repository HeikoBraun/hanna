...
pub static RE_COMMENT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)(?P<comment>--.*?)$").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_STD_LIBS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)^\s*library\s+(ieee|std)\b").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
pub static RE_LIBS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)^\s*library\s+(?P<name>\w+)\s*;").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
pub static RE_USE_STD_LIBS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)^\s*use\s+(ieee|std)\.\S+").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
pub static RE_USE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imx)^\s*use\s+(?P<content>(?P<lib>\w+\.)\S+?)\s*;").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_SIGNAL_OR_VARIABLE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)\b(?:constant|signal|variable)\b.*?;").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_GENERATE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imx)^.*\bgenerate\b.*?$").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_ENT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)((?P<lib>\w+)\.)?(?P<rest>(?P<entity>\w+)(\s*\(\s*(?P<arch>\w+)\s*\))?)")
        .unwrap_or_else(|err| {
            eprintln!("{}", err);
            exit(1)
        })
...
pub static RE_ENT2: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?P<lib>\w+)\.(?P<rest>.*)").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_USAGE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)(?P<lib>\w+)\.(?P<package>\w+)(\.(?P<element>\w+))?").unwrap_or_else(
        |err| {
            eprintln!("{}", err);
            exit(1)
        },
...
pub static RE_MODULE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)\bmodule\s+(?P<name>\w+)").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_MODULE_INST: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?imsx)\b(?P<name>\w+)\s+\w+\s+\(").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
...
pub static RE_ENVVAR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\$((?P<var1>\w+)|(\{(?P<var2>[^}]+?)}))").unwrap_or_else(|err| {
        eprintln!("{}", err);
        exit(1)
    })
});
